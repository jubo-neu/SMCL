...
def eval_step(steps, model, device, dataset, eval_dataset, batch, loss_fn, train_out, args, train_losses, eval_losses, eval_psnrs):
    step = steps[-1]
    ...
    with torch.no_grad():
        for height_start in range(0, H, args.eval.max_height):
            for width_start in range(0, W, args.eval.max_width):
                ...
                feature_map[:, height_start:height_end, width_start:width_end, :, :], attn[:, height_start:height_end, width_start:width_end, :, :] = model.evaluate(rayo, rayd[:, height_start:height_end, width_start:width_end], c2w, step=step)
    ...
    if args.eval.save_fig:
        ...
        main_plot = get_training_main_plot(args.index, steps, train_tgt_rgb, train_tgt_patch, train_pred_patch, test_tgt_rgb, test_pred_rgb, points_np, pt_plot_scale, depth, points_influ_scores_np)
    ...
    while step < args.training.steps:
        for _, batch in enumerate(trainloader):
            ...
            if (step % args.eval.step == 0) or (step % 500 == 0 and step < 10000):
                ...
                steps.append(step)
                eval_step(steps, model, device, dataset, eval_dataset, batch, loss_fn, out, args, train_losses, eval_losses, eval_psnrs)
                avg_train_loss = 0.
                eval_step_cnt = 0
