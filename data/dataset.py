...
class RINDataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        if coord_scale != 1:
            scaling_matrix = torch.tensor([[coord_scale, 0, 0, 0], [0, coord_scale, 0, 0], [0, 0, coord_scale, 0], [0, 0, 0, 1]], dtype=torch.float32)
        ...
        if self.args.extract_patch == True and self.args.extract_online == False and self.args.read_offline == True:
            ...
            self.rayo_patches = rayo_patches
    ...
    def __getitem__(self, idx):
        if self.args.extract_patch == True and self.args.extract_online == False and self.args.read_offline == True:
            ...
            return img_idx, patch_idx, self.img_patches[img_idx, patch_idx] if self.img_patches is not None else 0, self.rayd_patches[img_idx, patch_idx], self.rayo_patches[img_idx, patch_idx]
            ...
            if self.args.read_offline:
                img_patches, rayd_patches, rayo_patches, _ = extract_patches(self.images[img_idx:img_idx+1], self.rayo[img_idx:img_idx+1], self.rayd[img_idx:img_idx+1], args)
            else:
            ...
            return img_idx, 0, img_patches[0, 0] if img_patches is not None else 0, rayd_patches[0, 0], rayo_patches[0, 0]
        else:
            if self.args.read_offline:
                return idx, 0, self.images[idx] if self.images is not None else 0, self.rayd[idx], self.rayo[idx]
            else:
    ...
    def get_full_img(self, img_idx):
        if self.args.read_offline:
            return self.images[img_idx].unsqueeze(0) if self.images is not None else None, self.rayd[img_idx].unsqueeze(0), self.rayo[img_idx].unsqueeze(0)
        else:
            image, rayo, rayd = self._read_image_from_path(img_idx)
