import numpy as np
import torch
from .load_tanks_and_temple import load_tanksandtemple_data
from .load_nerf_synthetic import load_nerf_data
...
def cam_to_world(coords, c2w, vector=True):
    ...
    if coords.ndim == 5:
        ...
        transformed_coords = torch.sum(coords.unsqueeze(-2) * c2w.reshape(1, 1, 1, 1, 4, 4), -1)
    elif coords.ndim == 4:
        assert c2w.ndim == 3
        _, H, W, _ = coords.shape
        N = c2w.shape[0]
        transformed_coords = torch.sum(coords.unsqueeze(-2) * c2w.reshape(N, 1, 1, 4, 4), -1)
    elif coords.ndim == 3:
        assert c2w.ndim == 2
        H, W, _ = coords.shape
        transformed_coords = torch.sum(coords.unsqueeze(-2) * c2w.reshape(1, 1, 4, 4), -1)
    elif coords.ndim == 2:
        assert c2w.ndim == 2
        K, _ = coords.shape
        transformed_coords = torch.sum(coords.unsqueeze(-2) * c2w.reshape(1, 4, 4), -1)
    else:
...
def get_rays(H, W, focal_x, focal_y, c2w, fineness=1):
    N = c2w.shape[0]
    width = torch.linspace(0, W / focal_x, steps=int(W / fineness) + 1, dtype=torch.float32)
    height = torch.linspace(0, H / focal_y, steps=int(H / fineness) + 1, dtype=torch.float32)
    ...
    rays_d = cam_to_world(dirs_d.unsqueeze(0), c2w)
    rays_o = c2w[:, :3, -1]
...
def load_meta_data(args, mode="train"):
    ...
    if args.type == "synthetic":
        images, poses, hwf, image_paths = load_nerf_data(args.path, split=mode, factor=args.factor, read_offline=args.read_offline)
        print('Loaded nerf synthetic', images.shape, hwf, args.path)
        ...
        images, poses, hwf, image_paths = load_tanksandtemple_data(args.path, factor=args.factor, split=mode, read_offline=args.read_offline)
        print('Loaded tanks and temple', images.shape, hwf, args.path, images.min(), images.max(), images[0, 10, 10, :])
...
def rgb2norm(img):
    norm_vec = np.stack([img[..., 0] * 2.0 / 255.0 - 1.0, img[..., 1] * 2.0 / 255.0 - 1.0, img[..., 2] * 2.0 / 255.0 - 1.0, img[..., 3] / 255.0], axis=-1)
    return norm_vec
