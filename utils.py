...
def get_training_main_plot(index, steps, train_tgt_rgb, train_tgt_patch, train_pred_patch, test_tgt_tgb, test_pred_rgb, points_np, pt_plot_scale, depth_np, points_conf_scores_np=None):
    step = steps[-1]
...
def resample_shading_codes(shading_codes, args, model, dataset, img_id, loss_fn, step, full_img=False):
    ...
    sampled_shading_codes = torch.randn(args.exposure_control.shading_code_num_samples, args.exposure_control.shading_code_dim, device=model.device) * args.exposure_control.shading_code_scale
    ...
    with torch.no_grad():
        ...
        for i in range(args.exposure_control.shading_code_num_samples):
            ...
            foreground_rgb = model.renderer(feature_map.squeeze(-2).permute(0, 3, 1, 2), gamma=cur_gamma, beta=cur_beta).permute(0, 2, 3, 1).unsqueeze(-2)
