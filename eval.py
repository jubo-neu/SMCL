...
import vs_net.util
from vs_net.model import make_model
from vs_net.render import NeRFRenderer
import cv2
import tqdm
import ipdb
import warnings
...
args, conf = vs_net.util.args.parse_args(extra_args, default_conf="conf/Mir_180.conf", default_expname="shapenet")
args.resume = True
...
data_loader = torch.utils.data.DataLoader(dset, batch_size=1, shuffle=False, num_workers=0, pin_memory=False)
...
renderer = NeRFRenderer.from_conf(conf["renderer"], lindisp=dset.lindisp, eval_batch_size=args.ray_batch_size).to(device=device)
if args.coarse:
    net.mlp_fine = None
    ...
    for obj_idx, data in enumerate(data_loader):
        ...
        if args.scale != 1.0:
            ...
            if abs(Ht / args.scale - H) > 1e-10 or abs(Wt / args.scale - W) > 1e-10:
                warnings.warn("Inexact scaling, please check {} times ({}, {}) is integral".format(args.scale, H, W))
            H, W = Ht, Wt
        ...
        if has_output:
            ...
            for i in range(n_gen_views):
                ...
                if args.write_depth:
                    ...
                    out_depth_norm_file = os.path.join(obj_out_dir, "{:06}_depth_norm.png".format(novel_view_idxs[i].item()))
        ...
        if not args.no_compare_gt:
            ...
            for view_idx in range(n_gen_views):
                ...
                psnr = compare_psnr(all_rgb[view_idx], rgb_gt_all[view_idx], data_range=1)
                curr_ssim += ssim
                curr_psnr += psnr
