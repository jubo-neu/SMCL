...
def get_attention_layer(args, v_extra_dim=0, k_extra_dim=0, q_extra_dim=0, eps=1e-6, use_amp=False, amp_dtype=torch.float16):
    k_dim_map = {1: [3, 3, 3]}
    ...
    return Attention(d_k=k_dim, d_q=q_dim, d_v=v_dim, d_model=args.d_model, embed_args=args.embed, score_act=args.score_act, d_ko=k_extra_dim, d_qo=q_extra_dim, d_vo=v_extra_dim, eps=eps, use_amp=use_amp, amp_dtype=amp_dtype)
...
def attention(query, key, kernel_type):
    ...
    if kernel_type == "scaled-dot":
        ...
        scores = torch.matmul(query, key.transpose(-2, -1)) / (torch.norm(query, dim=-1, keepdim=True) * torch.norm(key, dim=-1, keepdim=True).transpose(-2, -1))
    else:
...
class FeedForward(nn.Module):
    def __init__(self, d_input, d_output, d_ff, n_layer=2, act="relu", last_act="none", dropout=0.1, norm="layernorm", residual=False, act_a=1.0, act_b=1.0, act_trainable=False, use_wn=False, eps=1e-6, skip_layers=[], half_layers=[]):
        ...
        self.mlp = MLP(d_input, n_layer, d_ff, d_output, act_type=act, last_act_type=last_act, use_wn=use_wn, a=act_a, b=act_b, trainable=act_trainable, skip_layers=skip_layers, half_layers=half_layers)
...
class Embeddings(nn.Module):
    def __init__(self, d_k, d_q, d_v, d_model, args, d_ko=0, d_qo=0, d_vo=0, eps=1e-6):
        ...
        self.embed_k = FeedForward(d_k, args.key.d_ff_out, args.key.d_ff, args.key.n_ff_layer, args.key.ff_act, args.key.ff_last_act, args.key.dropout_ff, args.key.norm, args.key.residual_ff, args.key.ff_act_a, args.key.ff_act_b, args.key.ff_act_trainable, args.key.use_wn, eps, args.key.skip_layers, args.key.half_layers)
        self.embed_q = FeedForward(d_q, args.query.d_ff_out, args.query.d_ff, args.query.n_ff_layer, args.query.ff_act, args.query.ff_last_act, args.query.dropout_ff, args.query.norm, args.query.residual_ff, args.query.ff_act_a, args.query.ff_act_b, args.query.ff_act_trainable, args.query.use_wn, eps, args.query.skip_layers, args.query.half_layers)
        self.embed_v = FeedForward(d_v, args.value.d_ff_out, args.value.d_ff, args.value.n_ff_layer, args.value.ff_act, args.value.ff_last_act, args.value.dropout_ff, args.value.norm, args.value.residual_ff, args.value.ff_act_a, args.value.ff_act_b, args.value.ff_act_trainable, args.value.use_wn, eps, args.value.skip_layers, args.value.half_layers)
...
class Attention(nn.Module):
    def __init__(self, d_k, d_q, d_v, d_model, embed_args, score_act="none", d_ko=0, d_qo=0, d_vo=0, eps=1e-6, use_amp=False, amp_dtype=torch.float16):
