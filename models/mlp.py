...
class MLP(nn.Module):
    def __init__(self, inp_dim=2, num_layers=3, num_channels=128, out_dim=2, act_type="leakyrelu", last_act_type="none", use_wn=True, a=1., b=1., trainable=False, skip_layers=[], bias=True, half_layers=[], residual_layers=[], residual_dims=[]):
        ...
        for i in range(num_layers):
            ...
            layers.append(wn(nn.Linear(cur_inp, cur_out, bias=bias), name='weight'))
            layers.append(activation_func(act_type=act_type, num_channels=cur_out, a=a, b=b, trainable=trainable))
        layers[-1] = activation_func(act_type=last_act_type, num_channels=out_dim, a=a, b=b, trainable=trainable)
...
class MappingMLP(nn.Module):
    def __init__(self, args, inp_dim=2, out_dim=2, use_amp=False, amp_dtype=torch.float16):
        ...
        self.model = MLP(inp_dim=inp_dim, num_layers=args.num_layers, num_channels=args.dim, out_dim=out_dim, act_type=args.act, last_act_type=args.last_act, use_wn=args.use_wn)
