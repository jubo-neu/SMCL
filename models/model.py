...
from .attentionblock2 import get_attention_layer
from .renderer import get_generator
from Point_GCL.encoder import PointEncoder
...
class SMCL_point(nn.Module):
    def __init__(self, args, device='cpu'):
        super(SMCL_point, self).__init__()
        self.args = args
        self.eps = args.eps
        self.device = device
        ...
        self.points_influ_scores = torch.nn.Parameter(torch.ones(points.shape[0], 1, device=device) * point_opt.influ_init_val, requires_grad=True)

        self.use_pc_feats = pc_feat_opt.use_ink or pc_feat_opt.use_inq or pc_feat_opt.use_inv

        if self.use_pc_feats:
            encoder = PointEncoder(input_dim=3, pretrained=True, pretrained_weights_path="./Point_GCL/encoder_barn.pth")   # 改
            encoded_points = encoder("./point_cloud/barn 10000.pcd")   # 改
            print("encoded_points", encoded_points.shape)
            repeat_factor = 10000 // encoded_points.size(0)
            upsampled_points = encoded_points.repeat(repeat_factor, 1)
            print("Upsampled points shape:", upsampled_points.shape)
            self.pc_feats = nn.Parameter(upsampled_points, requires_grad=True)
        ...
        if args.models.use_renderer:
            ...
            self.renderer = get_generator(args.models.renderer.generator, in_c=feat_dim, out_c=3, use_amp=self.use_amp, amp_dtype=self.amp_dtype)
        else:
        ...
        self.proximity_attn = get_attention_layer(args.models.attn, v_extra_dim=v_extra_dim, k_extra_dim=k_extra_dim, q_extra_dim=q_extra_dim, eps=self.eps, use_amp=self.use_amp, amp_dtype=self.amp_dtype)
    ...
    def init_optimizers(self, total_steps):
        ...
        self.optimizers = {"points": optimizer_points, "attn": optimizer_attn, "points_influ_scores": optimizer_points_influ_scores}

        self.schedulers = {"points": lr_scheduler_points, "attn": lr_scheduler_attn, "points_influ_scores": lr_scheduler_points_influ_scores}
    ...
    def _calculate_global_distances(self, rays_o, rays_d, points):
        ...
        _, select_k_ind = feature.topk(self.select_k, dim=-1, largest=False, sorted=False)
    ...
    def _calculate_distances(self, rays_o, rays_d, points, c2w):
        ...
        rays = normalize_vector(rays_d, eps=self.eps).unsqueeze(-2)
        v = points - rays_o.reshape(N, 1, 1, 1, 3)
    ...
    def _get_points(self, rays_o, rays_d, c2w, step=-1):
        ...
        if self.select_k >= points.shape[0] or self.select_k < 0:
            ...
            select_k_ind = self._calculate_global_distances(rays_o, rays_d, points)
    ...
    def add_points(self, add_num):
        ...
        new_points, num_new_points, new_influ_scores, new_point_features = add_points_knn(points, self.points_influ_scores.detach().cpu(), add_num=add_num, k=self.args.geoms.points.add_k, comb_type=self.args.geoms.points.add_type, sample_k=self.args.geoms.points.add_sample_k, sample_type=self.args.geoms.points.add_sample_type, point_features=point_features)
    ...
    def _get_kqv(self, rays_o, rays_d, points, c2w, select_k_ind, step=-1):
        ...
        if self.args.geoms.point_feats.use_inv:
            valid_indices = torch.clamp(select_k_ind, min=0, max=self.pc_feats.size(0) - 1)
            v_extra = [self.pc_feats[valid_indices, :]]
    ...
    def evaluate(self, rays_o, rays_d, c2w, step=-1, shading_code=None):
        ...
        if self.bkg_feats is not None:
            ...
            fused_features = torch.sum(embedv * topk_attn, dim=3, keepdim=True)
        else:
            attn = F.softmax(scores, dim=3)
            if self.args.models.normalize_topk_attn:
                attn = attn / torch.sum(attn, dim=3, keepdim=True)
            fused_features = torch.sum(embedv * attn, dim=3, keepdim=True)
    ...
    def forward(self, rays_o, rays_d, c2w, step=-1, shading_code=None):
        ...
        if step >= 0 and step % 200 == 0:
            print(' embedv:', step, embedv.shape, embedv.min().item(), embedv.max().item(), embedv.mean().item(), embedv.std().item())
            print(' scores:', step, scores.shape, scores.min().item(), scores.max().item(), scores.mean().item(), scores.std().item())
        ...
        if self.bkg_feats is not None:
            ...
            if self.args.models.normalize_topk_attn:
                topk_attn = topk_attn / torch.sum(topk_attn, dim=3, keepdim=True)
            fused_features = torch.sum(embedv * topk_attn, dim=3)

            if self.args.models.use_renderer:
                foreground = self.renderer(fused_features.permute(0, 3, 1, 2), gamma=gamma, beta=beta).permute(0, 2, 3, 1).unsqueeze(-2)
            else:
            ...
            attn = F.softmax(scores, dim=3)
            fused_features = torch.sum(embedv * attn, dim=3)
            if self.args.models.use_renderer:
                rgb = self.renderer(fused_features.permute(0, 3, 1, 2), gamma=gamma, beta=beta).permute(0, 2, 3, 1)
            else:
                rgb = fused_features

        if step >= 0 and step % 1000 == 0:
            print(' feat map:', step, fused_features.shape, fused_features.min().item(), fused_features.max().item(), fused_features.mean().item(), fused_features.std().item())
            print(' predict rgb:', step, rgb.shape, rgb.min().item(), rgb.max().item(), rgb.mean().item(), rgb.std().item())
    ...
    def save(self, step, save_dir):
        ...
        torch.save(scaler_state_dict, os.path.join(save_dir, 'scaler.pth'))
    ...
    def load(self, load_dir, load_optimizer=False):
        ...
        if os.path.exists(os.path.join(load_dir, 'scaler.pth')):
            scaler_state_dict = torch.load(os.path.join(load_dir, 'scaler.pth'))
