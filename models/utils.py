...
def add_points_knn(coords, influ_scores, add_num, k, comb_type="mean", sample_type="random", sample_k=10, point_features=None):
    ...
    if comb_type == "duplicate":
        ...
        nns_inds = nns_inds[:, 1:]
        if comb_type == "mean":
            new_coords = coords[nns_inds, :].mean(axis=-2)
...
def create_learning_rate_fn(optimizer, max_steps, args, debug=False):
    ...
    warmup_fn = lr_scheduler.LinearLR(optimizer, start_factor=warmup_start_factor, end_factor=1.0, total_iters=args.warmup, verbose=debug)

    if args.type == "linear":
        decay_fn = lr_scheduler.LinearLR(optimizer, start_factor=1.0, end_factor=0., total_iters=max_steps - args.warmup, verbose=debug)
        schedulers = [warmup_fn, decay_fn]
        milestones = [args.warmup]

    elif args.type == "cosine":
        cosine_steps = max(max_steps - args.warmup, 1)
        decay_fn = lr_scheduler.CosineAnnealingLR(optimizer, T_max=cosine_steps, verbose=debug)
        schedulers = [warmup_fn, decay_fn]
        milestones = [args.warmup]
        ...
        decay_fn = lr_scheduler.ExponentialLR(optimizer, gamma=args.gamma, verbose=debug)
        schedulers = [warmup_fn, decay_fn]
        milestones = [args.warmup]
    ...
    schedule_fn = lr_scheduler.SequentialLR(optimizer, schedulers=schedulers, milestones=milestones, verbose=debug)
