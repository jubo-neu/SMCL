...
class vgg16(nn.Module):
    def __init__(self, requires_grad=False, pretrained=True):
        ...
        if pretrained:
            vgg_pretrained_features = tv.vgg16(pretrained=True).features
        else:
            vgg_pretrained_features = tv.vgg16(pretrained=False).features
...
class LPNet(nn.Module):
    def __init__(self):
        ...
        model_path = os.path.abspath(os.path.join('.', 'vgg.pth'))
        print('Loading model from: %s' % model_path)
        weights = torch.load(model_path, map_location='cpu')
        for ll in range(self.L):
            self.lins[ll].weight = nn.Parameter(weights["lin%d.model.1.weight" % ll])
