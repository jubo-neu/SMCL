...
class Trainer:
    def __init__(self, net, train_dataset, test_dataset, args, conf, device=None):
        ...
        self.iter_state_path = "%s/%s/_iter" % (self.args.checkpoints_path, self.args.name)
        self.optim_state_path = "%s/%s/_optim" % (self.args.checkpoints_path, self.args.name)
        self.lrsched_state_path = "%s/%s/_lrsched" % (self.args.checkpoints_path, self.args.name)
        self.default_net_state_path = "%s/%s/net" % (self.args.checkpoints_path, self.args.name)
        ...
        if args.resume:
            ...
            if os.path.exists(self.iter_state_path):
                self.last_iter_id = torch.load(self.iter_state_path, map_location=device)["iter"]
                self.start_iter_id += self.last_iter_id + 1
                print(f"----------Resuming training from iteration {self.start_iter_id}----------")
