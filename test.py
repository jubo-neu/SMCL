...
def parse_args():
    parser = argparse.ArgumentParser(description="SMCL_point")
    parser.add_argument('--opt', type=str, default="", help='path to config file')
    parser.add_argument('--resume', type=int, default=250000, help='final step')
...
def test_step(frame, num_frames, model, device, dataset, batch, loss_fn, lpips_loss_fn_alex, lpips_loss_fn_vgg, args, test_losses, test_psnrs, test_ssims, test_lpips_alexs, test_lpips_vggs, resume_step):
    ...
    with torch.no_grad():
        ...
        if args.models.use_renderer:
            foreground_rgb = model.renderer(feature_map.squeeze(-2).permute(0, 3, 1, 2)).permute(0, 2, 3, 1).unsqueeze(-2)
        else:
        ...
        test_ssim = structural_similarity(rgb.squeeze().detach().cpu().numpy(), img.squeeze().detach().cpu().numpy(), win_size=11, multichannel=True, channel_axis=2, data_range=1.0)
    ...
    if args.test.save_video:
        ...
        if plot_opt.pcrgb:
            pcrgb_plot = get_test_pcrgb(frame, th, azmin, test_psnr, points_np, rgb_pred_np, rgb_gt_np, depth_np, pt_plot_scale, points_influ_scores_np)
            plots["pcrgb"] = pcrgb_plot

        if plot_opt.featattn:
            ...
            featattn_plot = get_test_featmap_attn(frame, th, points_np, rgb_pred_np, rgb_gt_np, pt_plot_scale, featmap_np, attn_np, points_influ_scores_np)
...
def test(model, device, dataset, save_name, args, resume_step):
    ...
    for frame, batch in enumerate(testloader):
        plots = test_step(frame, len(testloader), model, device, dataset, batch, loss_fn, lpips_loss_fn_alex, lpips_loss_fn_vgg, args, test_losses, test_psnrs, test_ssims, test_lpips_alexs, test_lpips_vggs, resume_step)
...
if __name__ == '__main__':
    ...
    with open("./conf/source3.yml", 'r') as f:
    ...
    sys.stdout = Logger(os.path.join(log_dir, 'metrics.log'), sys.stdout)
