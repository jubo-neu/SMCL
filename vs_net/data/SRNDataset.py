...
import numpy as np
from vs_net.util import get_image_to_tensor_balanced, get_mask_to_tensor


class SRNDataset(torch.utils.data.Dataset):
    def __init__(self, path, stage="train", image_size=(128, 128), world_scale=1.0):
        super().__init__()
        self.base_path = path + "_" + stage
        self.dataset_name = os.path.basename(path)

        print("Value of self.base_path:", self.base_path)

        assert os.path.exists(self.base_path), "Path does not exist: {}".format(self.base_path)
